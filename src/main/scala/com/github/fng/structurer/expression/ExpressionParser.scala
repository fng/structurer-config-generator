...
object ExpressionParser extends StandardTokenParsers {
  ...
  def unary_operation: Parser[Expression] = "(" ~> operator ~ expr <~ ")" ^^ {case o ~ e => new UnaryOperation(o, e)}

  def binary_operation: Parser[Expression] = "(" ~> expr ~ operator ~ expr <~ ")" ^^ {case l ~ o ~ r => new BinaryOperation(l, o, r)}

  def binary_operation_atoms: Parser[Expression] = expr ~ operator ~ expr <~ ")" ^^ {case l ~ o ~ r => new BinaryOperation(l, o, r)}

  def variable: Parser[Expression] = ident ^^ {s => KnownVariables.forName(s)}


  def nameConstant: Parser[Expression] = ident ^^ {s => new StringConstant(s)}


  def constant: Parser[Constant] = numericLit ^^ {s => new Constant(s.toDouble)}

  def operator: Parser[Operator] = plus | minus | mul | div

  def plus: Parser[Operator] = "+" ^^ {_ => Operator.ADD}

  def minus: Parser[Operator] = "-" ^^ {_ => Operator.SUB}

  def mul: Parser[Operator] = "*" ^^ {_ => Operator.MUL}

  def div: Parser[Operator] = "/" ^^ {_ => Operator.DIV}
}
